...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        ...
        case "create_custom_mode": {
            const newMode = request.params.arguments;
        ...
        case "get_custom_mode_fields": {
            ...
            const mode = roomodesContent.customModes.find((mode) => mode.slug === slug);
            if (!mode) {
        ...
        case "put_custom_mode_fields": {
            ...
            const modeIndex = roomodesContent.customModes.findIndex((mode) => mode.slug === slug);
            if (modeIndex === -1) {
                throw new Error(`Custom mode ${slug} not found`);
            }
